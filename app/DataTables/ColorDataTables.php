...
class ColorDataTables extends DataTable{
    ...
    public function dataTable($query)
    {
        return datatables($query)
            ->addColumn('edit', function ($model){
                $edit = '<a href="'. route('color.edit', $model->id) .'" class="btn btn-info">' . '<i class="fa fa-edit"></i>' . '</a>';
                return $edit;
            })
            ->addColumn('delete', function ($model){
                $edit = "<form action='". route('color.destroy', $model->id) ."' method='POST'>".
            ...
            ->addColumn('color', function ($model){
                return '<span style="width: 20px;height: 20px;display: block;background-color:'. $model->color .' "></span>';
